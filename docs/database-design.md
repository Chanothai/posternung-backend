...
| `sold_at` | TIMESTAMPTZ | NULL, CHECK (`ck_posters_sold_requires_sold_at`) | เวลาที่ **คนตัดสินว่าขายไปแล้ว** ไม่ใช่เวลาที่สคริปต์รัน (ADR-0025 D4) — 2 ทางเข้าเท่านั้น (§6 ด้านล่าง) เขียนพร้อม `status` ในทรานแซกชันเดียว (D1) · `NULL` = ยังไม่ถูกขาย **ไม่มี** `server_default` และ**ไม่ backfill** (D2 · AC-8) · 🔴 ต่างจาก `published_at`/`needs_review` ตรงที่ฟิลด์นี้**ออก public API** (`PosterDetailResponse` เท่านั้น — ADR-0013 Amendment A-D3) เพราะเป็นข้อเท็จจริงของสินค้า (คู่กับ `price`) ไม่ใช่ธง ops |
...
- 🔴 **`sold_at` เป็นแกนที่สาม (ADR-0025 D1/D2, migration `760b692b6062`, INF-24):** `status` ตอบ *"ซื้อได้ไหม"* · `published_at` ตอบ *"อยู่บนร้านไหม"* · `sold_at` ตอบ *"ขายเมื่อไหร่"* — คำถามที่สองไม่มีสิทธิ์ตอบคำถามแรกแทน (ADR-0013 Amendment A-D1) **ทั้งไฟล์ `app/services/poster_service.py` คือผู้เขียนเดียวของ `posters.status`** ในทั้งระบบ (ADR-0025 D5) ผ่าน 2 ทางเข้า — `mark_sold()` (ขายนอกระบบ) และ `mark_sold_by_order()` (order เข้า `COMPLETED` — INF-33 AC-4 สไลซ์ B, §6 ด้านล่าง) — ทั้งคู่เขียน `status=sold` + `sold_at` พร้อมกันเสมอในทรานแซกชันเดียว หลังล็อก `FOR UPDATE` แล้วตรวจว่าไม่มี reservation ที่ยัง `active` (ปฏิเสธทั้งรายการถ้ามี — ไม่มี `--force`) และ `status` เดิมต้องเป็น `available` (`mark_sold()`) หรือ `reserved` (`mark_sold_by_order()`) เท่านั้น
...
**`posters.status → sold` มี 2 ทางเข้า** (`app/services/poster_service.py` — ยังเป็น
ไฟล์เดียวตามเซตที่ ADR-0025 D5 ล็อกไว้): `mark_sold()` (ขายนอกระบบ · CLI operator ·
ADR-0025) และ `mark_sold_by_order()` (order เข้า `COMPLETED` · INF-33 AC-4 สไลซ์ B —
เรียกจาก `order_service.apply_order_transition()` หลัง `flush()` สถานะ `COMPLETED`
แล้ว ยังอยู่ใต้ล็อกเดียวกับที่ล็อก `posters` เป็นสมอไว้แล้ว)

**สิ่งที่ยังไม่มีหลังสไลซ์ B:** scheduler ทุกตัว (AC-7) · worker ส่งแจ้งเตือน (AC-8) ·
เส้นทางสร้างแถว `payments` · `orders.status → CANCELLED` ยังไม่ปล่อย listing กลับ
`available` เอง (เส้นทางยกเลิกเป็นของ SCR-07/SCR-15)
