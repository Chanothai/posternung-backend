...
- **`oauth_identities` แยกตาราง** จาก `users` — `provider_user_id` = **Firebase `uid`** (`sub` claim) เป็น key ที่เสถียรต่อ user ใน project ไม่ใช้ email เป็น key เพราะเปลี่ยนได้ · ผูก user เดิมที่มี email ตรงกันเข้ากับ identity ได้ และถ้ายังไม่ verify มาก่อน จะ auto-verify ให้ทันที
