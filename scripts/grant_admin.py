"""ตั้งสิทธิ์แอดมินให้บัญชีที่มีอยู่แล้ว (ADR-0031 D6)

‹แก้ 2026-08-25› เดิมเขียนว่า "ทางเดียวของการตั้งแอดมิน **คนแรก**" ซึ่งไม่ครบแล้ว —
`A1-D6` เพิ่มบัญชีแอดมินที่สองของเจ้าของคนเดิมผ่านสคริปต์นี้ด้วย `--allow-additional-admin`
**เพราะยังไม่มี endpoint แอดมินที่ให้สิทธิ์ได้** (จะมีที่ SCR-15) · D6-c ยังบังคับเหมือนเดิม
คือต้องใส่ flag ยืนยันชัดเจน ไม่ใช่เพิ่มได้เงียบ ๆ
...
D6-c  **ปฏิเสธถ้ามีแอดมินอยู่แล้ว** เว้นแต่ใส่ `--allow-additional-admin`
      กันไม่ให้สคริปต์ bootstrap กลายเป็นประตูหลังถาวร — การเพิ่มแอดมินต้องเป็นการ
      **ตั้งใจที่พิมพ์ออกมาได้** ไม่ใช่ผลข้างเคียงของการรันสคริปต์ซ้ำ
      ‹แก้ 2026-08-25› ถ้อยคำเดิมว่า "การเพิ่มแอดมินต้องไปผ่าน endpoint ที่มี
      `require_admin`" **ยังไม่จริงวันนี้** เพราะ endpoint นั้นยังไม่มี (SCR-15)
      ⇒ ระหว่างนี้สคริปต์ + flag คือทางที่ถูกต้อง ไม่ใช่ทางเลี่ยง
