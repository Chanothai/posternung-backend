...
`ai_suggest.py` เป็นตัวสำคัญที่สุด — มันรันด้วย **venv คนละตัว**
...
LANES: dict[str, Lane] = {
    ...
    "photo": Lane(
        8,
        "นำเข้ารูปจากโฟลเดอร์ในเครื่อง (ไม่มีใบงาน — อ่านชนิดจากชื่อไฟล์)",
        apply="photo_entry.py",
    ),
}
...
EXCLUDED: dict[str, str] = {
    ...
    # ‹2026-08-16 · INF-26 AC-9› `prepare_seed.py` และ `migrate_to_r2.py` เคยอยู่ที่นี่
    # — **ถูกลบออกจาก repo แล้วทั้งคู่** ตามคำสั่งเจ้าของ (สคริปต์ขั้นนำเข้าครั้งเดียว
    # จาก TikTok · input CSV ต้นทางไม่มีในเครื่องและติด `.gitignore` ⇒ รันไม่ผ่านตั้งแต่
    # เปิดไฟล์) · การถอด `prepare_seed.py` ต้องผ่าน **ADR-0019 A-D3** ก่อน เพราะ D9
    # เคยระบุชื่อมันไว้เป็นเจ้าของประตูนำเข้า
    # 🔴 เทส closed-world ด้านล่างเป็นตัวที่ฟ้องว่าต้องถอนชื่อออกจากที่นี่ด้วย
    # ไม่ใช่ลบไฟล์แล้วจบ
}
...
def _usage() -> str:
    lines = [
        ...
        "",
        "แปดเส้นทางของข้อมูลโปสเตอร์ (README.md §5 · ADR-0015 D1 — คนละแหล่ง คนละกฎ):",
        "",
    ]
