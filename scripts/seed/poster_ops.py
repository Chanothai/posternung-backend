...
EXCLUDED: dict[str, str] = {
    ...
    "prepare_seed.py": (
        ...
        "· ยังลบไม่ได้เพราะ make_triage_sheet.py import NOT_A_POSTER_REASON จากมัน "
        "และ ADR-0019 D9 ระบุชื่อไฟล์นี้เป็นเจ้าของประตูนำเข้า"
    ),
    # ‹2026-08-16› `migrate_to_r2.py` เคยอยู่ที่นี่ — ถูกลบทิ้งแล้วตามคำสั่งเจ้าของ
    # (อัปโหลดรูปครั้งเดียวขึ้น R2 · input `images-manifest.csv` ไม่มีในเครื่องแล้ว
    #  ⇒ รันไม่ผ่านตั้งแต่เปิดไฟล์) · เทส closed-world ด้านล่างเป็นตัวที่ฟ้องว่าต้องถอน
    # ชื่อออกจากที่นี่ด้วย ไม่ใช่ลบไฟล์แล้วจบ
}
