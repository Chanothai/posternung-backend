...
🔴 **G2 (code-critic รอบ 2 ของ INF-29)** — ย่อหน้านี้เคยเขียนว่า *"วันนี้ `publish=Y`
ผ่านได้ถ้าใบนั้นมีเกรดและมีรูป"* ซึ่งเป็นเท็จตั้งแต่ INF-28 merge (`verified_at`
ต้องมีลายเซ็นด้วย) และยิ่งเท็จมากขึ้นหลัง INF-27/INF-29 (ต้องมี `is_unique`/
`count_actual`/รูป `FRONT` ด้วย) — แก้ให้ตรงกับ `publish_blockers()` จริงแล้ว
...
อีกต่อไป** — ต้องผ่านเส้นที่ 5 มาเซ็นก่อนเสมอ (รายละเอียดเดียวกับที่อธิบายไว้ใน §เส้นที่ 5
ด้านล่าง)

กติกาที่ `publish=Y` ต้องผ่าน**ทั้งหมด**วันนี้คือนิยามเดียวที่
`poster_service.publish_blockers()` (`PublishReadiness` — ADR-0027 D3/D5) **ไม่ใช่แค่
เกรดกับรูปเหมือนก่อน INF-27/INF-28/INF-29 อีกต่อไป** — ตัวเงื่อนไขทุกข้อดูที่ฟังก์ชันนั้นเอง
ห้ามอ่านจากที่นี่แล้วเดาว่าครบ (`README` นี้ห้ามเป็นแหล่งความจริงที่สองของกฎ):
ต้องมีเกรด · ต้องมีลายเซ็น (`verified_at` ไม่เป็น `NULL` — มาจากเส้นที่ 5 เท่านั้น) ·
`is_unique` ต้องรู้ค่าและเป็น `true` · `count_actual` ต้องรู้ค่า ไม่เป็น 0 และไม่ ≥2 บน
เกรดที่ไม่ใช่ `mint` · ต้องมีรูป `kind=FRONT` อย่างน้อยหนึ่งรูป — 🔴 **ทุกฟิลด์ที่ยังไม่รู้
ค่า (`None`) = ไม่ผ่านเสมอ** (fail-closed, ADR-0027 D5) ไม่ใช่แปลว่า "ไม่เกี่ยวกับใบนี้"

⚠️ **นโยบาย SCR-11 (Condition Guide) ยังไม่มีด่านแยกต่างหากใน `publish_blockers()`** —
ฟังก์ชันนี้ไม่เช็คว่า SCR-11 เสร็จหรือยัง (ADR-0016 D8 ตัดสินแล้วว่านี่ไม่ใช่เงื่อนไขของ
การเปิดขาย — ดู `screens.yaml` SCR-11 · INF-11 `known_gaps` · ADR-0015 §ต้องทำตามมา)
