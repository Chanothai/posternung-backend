...
## 5. ห้าเส้นทางที่เขียน `posters` — คนละแหล่ง คนละกฎ (ADR-0015 D1)
...
🔴 **เส้นที่ 1–4 เติมช่องที่ยังว่าง · เส้นที่ 5 ทับค่าที่ลูกค้าเห็นไปแล้ว** — นั่นคือ
เส้นแบ่งที่สำคัญที่สุดในหน้านี้ และเป็นเหตุผลที่เส้นที่ 5 มีกติกาที่เส้นอื่นไม่มี
(ADR-0015 D1 Amendment 2026-08-09 · ADR-0010 A-D2)
...
แตะ database เหมือนกันทั้งสี่เส้นที่รับ `--reviewed-at`
...
ในสี่เส้นเพราะ `--reviewed-at` ถูกใช้เป็น `published_at` ของแถวที่ `publish=Y` ด้วย
...
🔴 **ค่าในวงเล็บมุมเป็น placeholder ที่ก๊อปแล้วรันไม่ผ่านโดยตั้งใจ** — เหมือนอีกสามเส้น

🔴 **เส้นเดียวที่ *ทับ* ค่าที่มีอยู่แล้ว** — อีกสี่เส้นเขียนได้เฉพาะช่องที่ยังว่าง
...
| `is_unique` (`Y`/`N`) + `is_unique_reason` | ทับค่าเดิม + audit 1 แถวพร้อมเหตุผล |
...
- 🔴 **`is_unique = N` เขียนได้เฉพาะแถวที่เกรดเป็น `mint`** (ADR-0019 **D1** —
  *"ไม่มีข้อยกเว้น ไม่มีดุลพินิจ"*) · วันนี้แปลว่า `N` เขียนไม่ได้เลยเพราะทั้งตาราง
  ไม่มีใบไหนเป็น `mint` — **นั่นคือผลที่ถูกต้อง ไม่ใช่บั๊ก** · ของหลายชิ้นต้อง
  **แตกเป็นหลายแถว** ซึ่งเป็นงานของ **INF-22**
...
- `--reviewed-at` **ไม่มี default เป็นเวลาปัจจุบัน** โดยตั้งใจ (ADR-0010 D5) — เวลาที่คน
  ...
  *"`reference_entry.py` เท่านั้นในตอนนี้"* ซึ่ง**ไม่จริงตั้งแต่ด่านถูกยกไป `_shared.py`**:
  ทั้งสี่เส้นที่รับ `--reviewed-at` ใช้ `assert_not_in_the_future()` **object เดียวกัน**
...
- **`production` ไม่มีให้เลือกในสคริปต์ตัวไหนเลย** — `--target` รับแค่ `dev|sit`
  (`manual_entry.py:TARGETS` · `reference_entry.py` และ `correction_entry.py` import
  ทูเพิลเดียวกันมาใช้ ไม่ประกาศซ้ำ) และ
