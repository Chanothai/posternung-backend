...
🔴 **เส้นเดียวที่ INSERT แถวใหม่ นอกจาก `seed_posters.py`** — ห้าเส้นก่อนหน้าล้วน UPDATE
...
- **UNIQUE ที่ `poster_splits.child_poster_id`** กันรันซ้ำสร้างลูกซ้ำที่ระดับ DB
  (ไม่ใช่แค่ระดับสคริปต์) — มีเทส DB-level ล็อกไว้แยกต่างหาก
