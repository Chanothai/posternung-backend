...
| `verified_at` | คำสั่ง | `KEEP` / `UNSIGN` | 🔴 อ่านออกแต่เขียนไม่ได้ — เว้นว่างแทน (ผลเหมือนกัน) |
| `published_at` | คำสั่ง | **`WITHDRAW`** + เหตุผล | ถอนของออกจากชั้น — `published_at := NULL` |
| `published_at` | คำสั่ง | `KEEP` / `PUBLISH` | 🔴 อ่านออกแต่เขียนไม่ได้ — การ *เปิด* ขายเป็นของเส้นที่ 3 ที่เดียว |
...
- ⚠️ **"อ่านไม่ออก" กับ "อ่านออกแต่ห้าม" เป็นคนละข้อความ** — ใช้กับทั้ง `is_unique=N`
  และ `KEEP`/`UNSIGN`/`PUBLISH` — parser ยัง parse ได้ตามปกติ (ไม่ใช่ค่ากำกวม) ส่วน
  ด่านนโยบายอยู่ที่ `parse_rows()` และอธิบายเหตุผลของตัวเอง · คนที่พิมพ์ค่าถูกตามที่
  เข้าใจต้องไม่ได้ข้อความว่าพิมพ์ผิด
