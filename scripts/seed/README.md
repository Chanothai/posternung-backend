...
🔴 **เส้นธุรกรรม (order/payment) ไม่อยู่ในโฟลเดอร์นี้** — อยู่ที่ `scripts/orders/`
(`order_ops.py` — INF-41) เพราะไม่เขียน `posters` เลยสักคอลัมน์โดยตรง ดูเหตุผลเต็มที่
`scripts/orders/README.md`
...
✅ **CHECK ระดับ DB ของ invariant `published ⇒ verified` ลงแล้ว**
(`ck_posters_published_requires_verified` — ADR-0027 Amendment 3 A3-D1 · INF-38)
บังคับทั้งฝั่ง Python (ด่านก่อนเซ็นข้างบน) และ DB พร้อมกัน — เว้นข้อยกเว้นเดียวคือ
ใบที่ `status = 'sold'` (ของที่ขายไปแล้วไม่มีใครหยิบขึ้นมาเซ็นย้อนหลังได้อีก A3-D2)
...
#### 🔎 Runbook — รอบนับของจริงหลังกู้ข้อมูล 2026-08-29

หลังกู้แคตตาล็อกชุดเดิม 117 ใบกลับมาและถอนทั้งชั้นแล้ว (`published_at` เหลือ 1 ใบ)
**เจ้าของต้องเดินนับของจริงทีละใบ** — และระหว่างนั้นจะเจอใบที่ **หยิบของจริงไม่เจอ**
เพราะขายไปช่วง **19–26 ส.ค. 2026** ซึ่งฐานชุดเดิมไม่รู้เรื่องด้วย (ถูก dump ไว้ก่อนหน้านั้น)

**ใบที่หยิบไม่เจอ ให้จดไว้ก่อน อย่าเพิ่งบันทึกกลางรอบ** แล้วบันทึกทีหลังด้วยเส้นนี้:

```bash
./venv/bin/python scripts/seed/sold_entry.py --commit \
    --poster-uuid <uuid ของใบที่หยิบไม่เจอ> \
    --sold-at <เวลาที่คาดว่าขายจริง ISO-8601 พร้อม timezone> \
    --reason "ไม่พบของจริงในรอบนับ YYYY-MM-DD · คาดว่าขายช่วง 19-26 ส.ค." \
    --reviewed-by <ชื่อคุณ> --reviewed-at <เวลาที่คุณตัดสิน>
```

🔴 **ห้าม `UPDATE posters SET status='sold'` ตรงเด็ดขาด** — `status` เป็น closed-world
ของ `poster_service` (ADR-0025 D5 · ADR-0033 D5) และการเขียนตรงจะ **ไม่มีแถว audit**
ซึ่งเป็นสิ่งเดียวที่ทำให้อีกหกเดือนยังตอบได้ว่าใบนี้หายไปไหนและใครตัดสิน

🔴 **`--sold-at` เป็นการคาดการณ์ ไม่ใช่ข้อเท็จจริงที่วัดได้** — ต่างจากใบ `ec3478a8`
ที่เจ้าของยืนยันวันขายได้จริง · เขียน `--reason` ให้บอกตรง ๆ ว่าเป็นการคาด เพื่อไม่ให้
คนอ่านย้อนเข้าใจว่าเป็นวันที่ยืนยันแล้ว (ทรงเดียวกับที่ ADR-0025 D4 แยก `sold_at`
ออกจาก `reviewed_at`)

**ลำดับที่ถูกในรอบนับหนึ่งใบ:**

| หยิบของจริงขึ้นมาแล้ว | ทำอะไร |
|---|---|
| เจอ · ตรงกับที่บันทึก | เส้นที่ 5 `verified_at = SIGN` → แล้วเปิดขายด้วยเส้นที่ 3 (`publish=Y`) |
| เจอ · แต่เกรด/จำนวนไม่ตรง | เส้นที่ 5 แก้ค่า **แล้วจึงเซ็น** (แก้ค่าล้างลายเซ็นเองตาม ADR-0027 D6) |
| **หยิบไม่เจอ** | **เส้นนี้ (`sold_entry.py`) ตามคำสั่งข้างบน** — ไม่ต้องเซ็น ไม่ต้องเปิดขาย |

⚠️ **ใบที่ยังไม่ได้นับ = ยังไม่ขึ้นร้าน โดยอัตโนมัติ** ไม่ต้องทำอะไรเพิ่ม — `published_at`
ถูกถอนไปแล้วทั้งชั้น และด่าน `is_publishable()` บล็อกที่ `verified_at` เป็น NULL อยู่แล้ว
⇒ **ความเสี่ยงขายของที่ไม่มีอยู่จริง ถูกกันด้วยด่าน ไม่ใช่ด้วยความจำของคนนับ**
